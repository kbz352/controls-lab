...
filename = f"ZN-{setpoint}-data-run-{run}"  # name of csv file to be stored in the data/ directory
f = open(f"/home/pi/controls-lab/comp-data/{filename}.csv", "w")
f.write("Time,RPM Setpoint,Left RPM,Right RPM,Left Power,Right Power\n")
f.write(
    f"{t},{setpoint},{l_RPM},{r_RPM},{l_power},{r_power},{l_distance},{r_distance}\n"
)
...
while t < duration:
    ...
    if t >= t_sample:
        ...
        print(
            f"time: {t:.02f} setpoint: {setpoint:.02f} ----- l_RPM: {l_RPM:.02f} r_RPM: {r_RPM:.02f} ----- l_power: {l_power:.02f} r_power: {r_power:.02f} ----- l_distance: {l_distance:.02f} r_distance: {r_distance:.02f}"
        )
        f.write(
            f"{t},{setpoint},{l_RPM},{r_RPM},{l_power},{r_power},{l_distance},{r_distance}\n"
        )
